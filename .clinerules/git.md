...
- **When an explicit instruction/workflow file is attached:** Include `<workflow id="{workflow_name}" />` line before human input
  - Extract the workflow name from the `type` attribute of the `<explicit_instructions>` tag
  - Example: `<workflow id="version-bump.md" />`
  - This identifies which workflow guided the AI's actions
- **ALWAYS append** the human input that led to the commit after the `---` separator
  - If the human's prompt is short (~255 words or less), prepend with `Prompt: ` and include the full raw prompt text
  - If the human's prompt is long, prepend with `Summary: ` and summarize to approximately one paragraph in length
